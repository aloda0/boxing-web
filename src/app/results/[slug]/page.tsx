...
import { getCompetitionResultBySlug } from "@/lib/data";
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-14 lg:px-6 lg:py-20">
      <div className="surface-matte rounded-3xl p-6 sm:p-8">
        ...
        <h1 className="mt-4 text-4xl font-bold tracking-tight text-white">{item.title}</h1>
        {item.location ? <p className="mt-4 text-zinc-400">Место: {item.location}</p> : null}

      {hasNewText ? (
        <GlassCard className="mt-10">
          <h2 className="text-lg font-semibold text-white">Результаты</h2>
        ...
        <>
          ...
          {rows.length > 0 ? (
            <GlassCard className="mt-10">
              <h2 className="text-lg font-semibold text-white">Список результатов</h2>
          ...
          {item.winners ? (
            <GlassCard className="mt-6">
              <h2 className="text-lg font-semibold text-white">Победители и призёры</h2>
              <pre className="mt-4 whitespace-pre-wrap font-sans text-sm leading-relaxed text-zinc-300">
                {item.winners}
              </pre>
            </GlassCard>
